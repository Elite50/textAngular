...
.factory('taExecCommand', ['taSelection', 'taBrowserTag', '$document', function(taSelection, taBrowserTag, $document){
  return function iframeDocument(iframeDoc) {
		var selection = taSelection(iframeDoc);
		var listToDefault = function (listElement, defaultWrap) {
			...
			listElement.remove();
			selection.setSelectionToElementEnd($target[0]);
		};
		var selectLi = function (liElement) {
			if (/(<br(|\/)>)$/i.test(liElement.innerHTML.trim())) selection.setSelectionBeforeElement(angular.element(liElement).find("br")[0]);
			else selection.setSelectionToElementEnd(liElement);
		};
		...
		return function (taDefaultWrap, topNode) {
			...
			return function (command, showUI, options) {
				...
				try {
					selectedElement = selection.getSelectionElement();
				} catch (e) {
				}
				...
				if (selectedElement !== undefined) {
					...
					if (command.toLowerCase() === 'insertorderedlist' || command.toLowerCase() === 'insertunorderedlist') {
						...
						var _selection = selection.getSelection();
						if (_selection.collapsed) {
							// insert text at selection, then select then just let normal exec-command run
							selection.insertHtml('<a href="' + options + '">' + options + '</a>', topNode);
							return;
						}
					} else if (command.toLowerCase() === 'inserthtml') {
						selection.insertHtml(options, topNode);
						return;
					}
				}
				try {
					var doc = iframeDoc || $document[0];
					doc.execCommand(command, showUI, options);
				} catch (e) {
				}
			};
		};
...
function($window, $document){
	return function iframeDoc(iframeDoc) {
		// need to dereference the document else the calls don't work correctly
		var _document = iframeDoc || $document[0];
